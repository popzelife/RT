...
/*   Updated: 2017/02/20 17:40:18 by nkhouide         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		draw_menu(t_rt *rt)
{
	...
	*/
	...
	surfparam(&rect, 0, &rt->panel.imgparam, 0), rt->esdl->eng.render,
	esdl_load_texture);

	rect = esdl_fuse_rect(*(rt->r_menu), esdl_rect(64 + 27 * 2, 520, 0, 0));
	set_imgparam(&rt->panel.imgparam, IMG_ADDPLANE);
	rt->panel.lst_surf = lst_new_image(&(rt->panel.lst_surf), surfparam(&rect,
	0, &rt->panel.imgparam, 0), rt->esdl->eng.render, esdl_load_texture);
	...
	*/
	...
	if (rt->panel.viewparam.scene.this_obj->type_obj == OBJ_SPHERE)
	{
		...
		sprintf(rt->panel.viewparam.str_pos, "Position: %5s[%.3g; %.3g; "
		"%.3g]", "", sphere->center.x, sphere->center.y, sphere->center.z);
	...
	"", rt->panel.viewparam.scene.this_obj->p_mat->albedo.x * 255.0,
	rt->panel.viewparam.scene.this_obj->p_mat->albedo.y * 255.0,
	rt->panel.viewparam.scene.this_obj->p_mat->albedo.z * 255.0);
...
void		reset_menu(t_rt *rt)
{
	...
	*/
	...
	rect = esdl_fuse_rect(*(rt->r_menu), esdl_rect(27, 520, 0, 0));
	rt->panel.lst_surf = lst_new_image(&(rt->panel.lst_surf),
	surfparam(&rect, 0, &rt->panel.imgparam, 0),
