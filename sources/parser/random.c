...
static void init_scene_sphere(t_rt *rt, t_scene *scene)
{
  ...
  scene->cam[0] = set_camera(
      v3_(13., 2., 3.), v3_(0., 0., 0.), v3_(0., -1., 0.),
      camparam(20., (double)rt->r_view->w / (double)rt->r_view->h, .1,
...
{
  scene->obj[*i] = new_object(
      (void *)new_sphere(v3_(0., 1., 0.), 1.),
      OBJ_SPHERE, new_material(v3_(1., 1., 1.), 1.5, NULL), MAT_DIELECT);
  *i += 1;
  scene->obj[*i] = new_object(
      (void *)new_sphere(v3_(-4., 1., 0.), 1.),
      OBJ_SPHERE, new_material(v3_(.4, .2, .1), 0., new_texture(TEXT_LINEY, NULL)), MAT_LAMBERT);
  *i += 1;
...
static void random_sphere(t_scene *scene, int a, int b, int *i)
{
  ...
  if (v3_length(v3_sub_vec_(center, v3_(4., .2, 0.))) > .9)
  {
    if (choose_mat < .8)
      scene->obj[*i] = new_object(
          (void *)new_sphere(center, .2),
          OBJ_SPHERE, new_material(v3_(f_rand() * f_rand(), f_rand() * f_rand(),
                                       f_rand() * f_rand()),
                                   0., NULL),
          MAT_LAMBERT);
      ...
      scene->obj[*i] = new_object(
          (void *)new_sphere(center, .2),
          OBJ_SPHERE, new_material(v3_(.5 * (1 + f_rand()), .5 * (1 + f_rand()),
                                       .5 * (1 + f_rand())),
                                   .5 * f_rand(), NULL),
          MAT_METAL);
    else
