...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/22 10:59:47 by qfremeau          #+#    #+#             */
/*   Updated: 2017/03/23 13:31:21 by qfremeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		bc_dielectric(t_scene *s, t_parser *p, char *line)
{
	...
	if (check_opt(p->opt_m) != E_TAB_DIELECT)
	{
		ft_printf("XML %s ERROR - Some flags are missing within <dielectric> "
...
void		bo_dielectric(t_scene *s, t_parser *p, char *line)
{
	...
	s->obj[p->i_obj].p_mat = (void*)new_material(v3_(1., 1., 1.), 1.1, NULL);
	s->obj[p->i_obj].p_mat->type_mat = MAT_DIELECT;
	p->f = (void*)&bo_void;
	p->opt_m |= p->byte[E_TAB_DIELECT];
	p->lim_mat++;
}
