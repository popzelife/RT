...
static void check_scene(t_rt *rt, t_scene *scene)
{
  if (scene->sizeof_cam == 0)
  {
    scene->sizeof_cam = 1;
    rt->parser.lim_cam = 1;
    if ((scene->cam = (t_cam *)realloc(scene->cam, scene->sizeof_cam * sizeof(t_cam))) == NULL)
      exit(-1);
    scene->cam[0] = set_camera(
        v3_(-5., 1., -10.), v3_(0., 0., 0.), v3_(0., -1., 0.),
        camparam(60., (double)rt->r_view->w / (double)rt->r_view->h,
                 .0, v3_length_double_(v3_sub_vec_(v3_(13., 2., 3.),
                                                   v3_(0., 0., 0.)))));
  ...
  if (scene->sizeof_skb == 0)
  {
    scene->sizeof_skb = 1;
    rt->parser.lim_skb = 1;
    if ((scene->skybox = (t_skybox *)realloc(scene->skybox,
                                             scene->sizeof_skb * sizeof(t_skybox))) == NULL)
      exit(-1);
