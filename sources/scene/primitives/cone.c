...
static void discriminant_cone(t_cone *c, const t_ray ray, t_discriminant *d)
{
  ...
  d->a = v3_dot_double_(ray.dir, ray.dir) - (1 + c->tang * c->tang) * v3_dot_double_(ray.dir, c->vertex) * v3_dot_double_(ray.dir, c->vertex);
  d->b = 2 * (v3_dot_double_(ray.dir, d->oc) - (1 + c->tang * c->tang) * v3_dot_double_(ray.dir, c->vertex) * v3_dot_double_(d->oc, c->vertex));
  d->c = v3_dot_double_(d->oc, d->oc) - (1 + c->tang * c->tang) * v3_dot_double_(d->oc, c->vertex) * v3_dot_double_(d->oc, c->vertex);
