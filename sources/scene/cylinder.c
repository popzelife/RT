...
/*   Updated: 2017/02/27 15:55:16 by qfremeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
BOOL		normal_cylinder(t_cylinder *cylinder, const t_ray ray,
		const float sol, t_hit *param)
{
	param->t = sol;
	param->pos = ray_point_at(ray, param->t);
	param->normal = v3_normalize_(v3_sub_vec_(param->pos, v3_sub_vec_(
	cylinder->cp, v3_scale_vec_(cylinder->vertex, cylinder->m))));
	return (TRUE);
}
