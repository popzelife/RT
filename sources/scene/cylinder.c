...
/*   Updated: 2017/02/28 19:09:30 by qfremeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

t_cylinder	*new_cylinder(t_vec3 vertex, t_vec3	cp, const double radius, const double height)
{
	t_cylinder	*c;
		...
		t_hit *param)
{
	...
	parta = v3_sub_vec_(param->pos, cylinder->cp);

	partb = v3_scale_vec_(cylinder->vertex, cylinder->m);
	...
	if (discriminant >= 0)
	{
		...
		if (sol < t[1] && sol > t[0])
		{
			cylinder->m = v3_dot_double_(ray.dir, v3_scale_vec_(cylinder->vertex, sol))
					+ v3_dot_double_(oc, cylinder->vertex);
//			if (cylinder->m >= 0 && cylinder->m <= cylinder->height)
