...
static void help_normal(t_dielec *d, const t_ray ray, const t_hit param)
{
             ...
             t_vec3 *refracted)
{
  ...
  if (discriminant > 0.0)
  {
    *refracted = v3_sub_vec_(v3_scale_vec_(
                                 v3_sub_vec_(uv, v3_scale_vec_(n, dt)),
                                 ni_over_nt),
                             v3_scale_vec_(n, sqrt(discriminant)));
    return (true);
  }
  else
    return (false);
