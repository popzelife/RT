...
t_ray ray_from_cam(t_cam *cam, double s, double t)
{
  ...
  offset = v3_add_vec_(v3_scale_vec_(cam->u, radius.x),
                       v3_scale_vec_(cam->v, radius.y));
  orig = v3_add_vec_(cam->origin, offset);
  dir = v3_sub_vec_(
      v3_sub_vec_(
          v3_add_vec_(
              v3_add_vec_(cam->low_left_corner,
                          v3_scale_vec_(cam->horizontal, s)),
              v3_scale_vec_(cam->vertical, t)),
          cam->origin),
      offset);
