...
static void prim_surface(t_rt *rt, t_menu *m)
{
  ...
  rt->panel.lst_surf = lst_new_surface(
      &(rt->panel.lst_surf), surfparam(
                                 &m->rect, 0xff888888, NULL, 0),
      rt->esdl->eng.render, esdl_clear_surface);
  m->rect = esdl_rect(5, 0, 191, TILE_RY - 10);
  set_imgparam(&rt->panel.imgparam, "image/Title.bmp");
  rt->panel.lst_surf = lst_new_image(
      &(rt->panel.lst_surf), surfparam(
                                 &m->rect, 0, &rt->panel.imgparam, 0),
      rt->esdl->eng.render,
      esdl_load_texture);
}
